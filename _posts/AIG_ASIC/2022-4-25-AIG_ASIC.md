...
header-img: "img/post-bg-ASIC.jpg"
header-mask: 0.4
catalog:      true 
tags:
    - ASIC
...
5.`case`是一种<font color=red>无优先级</font>的判断结构。与单`if` 语句的区别是，`case` 的条件互斥。多用于指令译码电路。
>此处zk认为`case`是有优先级的结构，但是在综合时可以综合成并行结构：[单击前往](https://blog.csdn.net/chief_cf/article/details/120370016)
